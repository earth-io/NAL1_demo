...
var Model=(__,self)=>{
  ...
  */
  ...
  var opCnt=0;
  var ops=[    // d.setState notifies observers - which only exist when run server.js
  ...
  var self={
    ...
    links:[],
    observerCBs:[],
    addObserver:(cb)=> self.observerCBs.push(cb),  // fire this every time something changes
    configure:(nCol_,nRow_,__,configureLinks)=>{  // initialization
      self.nCol=nCol_;
      self.nRow=nRow_;
...
//var model=Model(); model.configure( 5,4); range(31).map(()=> model.doRandomOp()); // in main
