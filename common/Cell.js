...
var Cell=function(k,__,self,getOtherCell,getOtherPort){
  ...
  self={ 
    ...
    setState:(state,__,state0)=>{   // FSM  needed - suggestions please  sends .update msg to links
      ...
      if(state!=state0){ 
        send(JSON.stringify({ cell_update:(JSON.stringify({ k:self.k, state:self.state, }) ), })  );
      }; // publish
      return self.state; 
    },
    propagateTreeOnPort:(treeID,port_out,cnt,__,link,otherCell,otherPort)=>{ 
      ...
      if(otherCell.trees[treeID]==undefined){    // trees is   
      //console.log( link_observers)           // never called  ******
      ...
      //    {cell1:self,port1:port_out,cell2:otherCell,port2:otherPort,tree:treeID,cnt:cnt});
        ...
        send(JSON.stringify({ tree_branch:(JSON.stringify({ treeID:treeID, linkID:link.id, state:link.state }) ), })  );
